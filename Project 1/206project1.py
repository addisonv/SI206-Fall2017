...
# [('Senior', 26), ('Junior', 25), ('Freshman', 21), ('Sophomore', 18)]
	...
	for x in data:
		if x["Class"] not in classes:
			...
			classes[x["Class"]] += 1
	classes_tup = classes.items()
	output = sorted(classes_tup, key=lambda x: x[1], reverse=True)
	return output 
...
# most often seen in the DOB
	...
	for date in a:
		...
		dobSplit = dobSpaces.split()
		if dobSplit[1] not in commonDays:
			commonDays[dobSplit[1]] = 1
		else:
			commonDays[dobSplit[1]] += 1 
	dobSorted = sorted(commonDays.items(), key=lambda x: x[1], reverse=True)
	output = int(dobSorted[0][0])
	return output
